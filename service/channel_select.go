...
// triedChannels is the set of channels this request has already been served by.
// The retry loop records each one before it attempts it (addUsedChannel), and
// until now that record was only ever read back to write a log line.
func (p *RetryParam) triedChannels() map[int]bool {
	if p == nil || p.Ctx == nil {
		return nil
	}
	used := p.Ctx.GetStringSlice("use_channel")
	if len(used) == 0 {
		return nil
	}
	tried := make(map[int]bool, len(used))
	for _, s := range used {
...
func (p *RetryParam) RetryBudget() int {
	...
	budget := total - 1
	// An operator who sets a ceiling means it; zero, the default, leaves the
	// bound to the pool. It can only lower the count: selection hands out each
	// channel once, so a number above the pool size has nothing left to try.
	if common.RetryTimes > 0 && budget > common.RetryTimes {
