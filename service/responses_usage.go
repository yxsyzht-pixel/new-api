...
// RawUsage is what upstream actually reported, and OutputTextLen how much
// generated text was seen. A caller deciding whether the stream delivered
// anything at all has to ask before Finish, which deliberately estimates a
// prompt charge for any stream that started and would make an empty turn look
// like a billable one.
func (a *ResponsesUsageAccumulator) RawUsage() *dto.Usage {
	if a == nil {
		return nil
	}
	return a.usage
...
func (a *ResponsesUsageAccumulator) OutputTextLen() int {
	if a == nil {
		return 0
	}
	return a.outputText.Len()
}
