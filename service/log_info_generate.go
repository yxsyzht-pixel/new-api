...
func appendStreamStatus(relayInfo *relaycommon.RelayInfo, other *model.LogOther) {
	...
	// same thing about the same stream. Upstream still folds the two together
	// here; keeping Outcome() is deliberate, so a merge must not take theirs.
	status := string(ss.Verdict())
