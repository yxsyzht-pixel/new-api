...
var defaultModelRatio = map[string]float64{
	...
	"gpt-image-2":                             2.5,  // $5 / 1M tokens，与 gpt-image-1 同价
	"o1":                                      7.5,  // $15 / 1M tokens
	...
	"gpt-5.6-terra":                           1.25,
	"gpt-5.6-luna":                            0.5,
	// Codex calls this one itself to vet auto-approvals; it is hidden from the
	// model picker, so nothing else would ever add it and it would bill at the
	// 37.5 fallback. Priced as the lightest 5.6 tier, which is what it behaves like.
	"codex-auto-review":                   0.5,
	"gpt-3.5-turbo":                       0.25,
...
var defaultCompletionRatio = map[string]float64{
	...
	"gpt-image-1":    8,
	"gpt-image-2":    8,
	// The 5.6 models get their completion ratio of 6 from the "gpt-" prefix rule
	// in getHardcodedCompletionModelRatio. codex-auto-review runs on the same
	// backend but its name does not start with "gpt-", so without an entry here it
	// would quietly settle at 1 and undercount every review it performs.
	"codex-auto-review": 6,
}
