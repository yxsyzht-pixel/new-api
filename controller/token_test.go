...
func verifyAPITokenAudit(t *testing.T) {
	...
	for index, tc := range []struct {
		...
		// Upstream expects this to succeed. In this fork a regular user must pick a
		// staff id out of the personnel directory (checkStaffDirectorySelection),
		// and no directory is configured under test, so the attempt is refused —
		// what stays covered is that a refused create is still audited, carries the
		// name, and leaves no key behind.
		{name: "create", method: "POST", path: "/", body: `{"name":"created","expired_time":-1,"unlimited_quota":true}`, action: "token.create"},
		...
		// Upstream lets a batch delete succeed on whatever it can reach. Ours is
		// all-or-nothing for a creator-scoped caller — BatchDeleteTokens refuses
		// with "只能删除自己创建的密钥" when the ids do not all resolve — so the
		// request is turned away and no count is recorded. Deliberate: silently
		// skipping keys the caller may not delete is how a partial delete reads
		// as a full one.
		{name: "batch delete partial and duplicate", method: "POST", path: "/batch", body: `{"ids":[$id,$id,$other,999999]}`, action: "token.delete_batch", params: `{"requested_ids":[$id,$id,$other,999999],"total":4}`},
		...
		t.Run(tc.name, func(t *testing.T) {
			empty := ""
			// CreatedBy is ours: mutations are scoped by creator rather than owner
			// (CreatorScope), so a key seeded without it cannot be updated or
			// deleted by anyone and every case below would fail to find it. The
			// foreign key deliberately stays without one, which is what keeps the
			// cross-account cases refusing.
			owned := &model.Token{UserId: user.Id, CreatedBy: user.Id, Name: "owned", Key: fmt.Sprintf("owned-key-secret-%d", index), Status: common.TokenStatusEnabled, ExpiredTime: -1, RemainQuota: 100, UnlimitedQuota: true, Group: "auto", CrossGroupRetry: true, AutoGroups: `["default"]`, AllowIps: &empty}
			...
			require.NoError(t, err)
			if tc.name == "create" {
				assert.JSONEq(t, `{"name":"created"}`, string(params))
				var created int64
				require.NoError(t, model.DB.Model(&model.Token{}).Where("user_id = ? AND name = ?", user.Id, "created").Count(&created).Error)
				assert.Zero(t, created, "a refused create must not leave a key behind")
			} else if tc.params == `{}` {
	...
	t.Run("bounded batch metadata", func(t *testing.T) {
		...
		for _, path := range []string{"/api/token/batch", "/api/token/batch/keys"} {
			...
			// None of these ids exist, and a creator-scoped batch delete in this
			// fork is all-or-nothing, so the delete path is refused here too —
			// upstream expects it to succeed with a count of zero.
	...
	t.Run("audit storage failure preserves operation result", func(t *testing.T) {
		...
		// Upstream drives this with a create. A regular user cannot create a key in
		// this fork without a directory-backed staff id, so a rename stands in: the
		// point is that a failed audit write leaves the operation itself intact,
		// and any audited mutation shows that.
		blank := ""
		seeded := &model.Token{UserId: user.Id, CreatedBy: user.Id, Name: "audit-down-before", Key: "audit-down-key-secret", Status: common.TokenStatusEnabled, ExpiredTime: -1, UnlimitedQuota: true, Group: "auto", AutoGroups: `["default"]`, AllowIps: &blank}
		require.NoError(t, model.DB.Create(seeded).Error)
		body := fmt.Sprintf(`{"id":%d,"name":"audit-down","unlimited_quota":true,"expired_time":-1,"group":"auto","allow_ips":""}`, seeded.Id)
		request := httptest.NewRequest("PUT", "/api/token/", strings.NewReader(body))
		request.Header.Set("Authorization", "Bearer "+jwt)
		request.Header.Set("Content-Type", "application/json")
		response := httptest.NewRecorder()
		router.ServeHTTP(response, request)
