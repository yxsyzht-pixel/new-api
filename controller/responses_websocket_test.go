	...
	for _, transport := range []string{"websocket", "http-sse"} {
		for _, tc := range []struct {
			...
			t.Run(transport+"/"+tc.name, func(t *testing.T) {
				if tc.failed {
					// A deliberate divergence, not a regression. Upstream hands an
					// in-stream failure straight to the caller and settles it as a
					// billable turn. Here it is withheld while nothing has been
					// delivered yet, so the relay loop can try a sibling account:
					// an in-stream `error` is how a busy Codex account says "not
					// me", and on 2026-09-20 that was 86% of attempts, nearly all
					// of them served by a later account. A turn nobody served is
					// not billed either. The withholding has its own coverage in
					// relay/channel/openai/responses_stream_failure_test.go, and
					// the two non-failing cases below still guard the accounting
					// this test was written for.
					t.Skip("gateway retries in-stream failures instead of forwarding them")
				}
...
func TestResponsesHTTPHealthCountsFinalResult(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			if tc.attempts > 1 {
				// Upstream retries a failed attempt on the same channel, so
				// RetryTimes alone buys another try. Here selection hands out each
				// channel once and the bound is the pool: this fixture has a single
				// channel, so the transient 500 is final and the caller is told at
				// once instead of waiting for a repeat of the same credential. With
				// a real pool the sibling accounts are what the retry reaches, which
				// TestAStaleReasoningReferenceIsRetriedOnce and the budget tests in
				// service/ cover.
				t.Skip("retries here are bounded by the channel pool, not a fixed count")
			}
