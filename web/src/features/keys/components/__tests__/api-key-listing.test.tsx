...
// Upstream's keys page merges creation and last use into one "Time" column and
// puts an Enable/Disable button directly on the row. This fork keeps the two
// dates as separate columns (with a stale-after-three-months highlight) and
// keeps status changes in the row menu, so these cases describe a layout that
// does not exist here. Skipped rather than deleted: they are the marker for a
// divergence to re-decide the next time the keys page is reworked.
it.skip('combines creation and last use while keeping expiry, models and IP restrictions separate', async () => {
...
// does not exist here. Skipped rather than deleted: they are the marker for a
// divergence to re-decide the next time the keys page is reworked.
it.skip('restores dates hidden by the old default and preserves unrelated column preferences', async () => {
...
// does not exist here. Skipped rather than deleted: they are the marker for a
// divergence to re-decide the next time the keys page is reworked.
it.skip('keeps expired status when the server refuses reactivation', async () => {
