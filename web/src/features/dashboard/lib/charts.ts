...
function formatCompactTokens(raw: number): string {
  ...
  if (absolute >= 1_000_000_000_000) {
    return `${formatter.format(raw / 1_000_000_000_000)} 万亿`
  }
  if (absolute >= 100_000_000) {
    return `${formatter.format(raw / 100_000_000)} 亿`
  }
  if (absolute >= 10_000) {
    return `${formatter.format(raw / 10_000)} 万`
  }
  return formatter.format(raw)
}

function formatExactTokens(raw: number): string {
...
export function processUserChartData(
  ...
  const formatVal = (raw: number) => {
    if (!isTokenMetric) return renderQuotaCompat(raw, 2)
    return formatCompactTokens(raw)
  }
  const formatValWithUnit = (raw: number) => {
    const formatted = formatVal(raw)
    return isTokenMetric ? `${formatted} ${tt('Tokens')}` : formatted
  }
  const formatTooltipValWithUnit = (raw: number) => {
    const formatted = formatValWithUnit(raw)
    if (!isTokenMetric || Math.abs(raw) < 10_000) return formatted
    return `${formatted}（${formatExactTokens(raw)}）`
  }
  const getMetricValue = (item: QuotaDataItem) => Number(item[metricField]) || 0
          ...
          content: [
            {
              ...
              value: (datum: Record<string, unknown>) =>
                formatTooltipValWithUnit(Number(datum?.rawValue) || 0),
            },
          ],
          updateContent: (
            array: Array<{
            ...
            for (let i = 0; i < array.length; i++) {
              ...
              array[i].value = formatTooltipValWithUnit(Number(value) || 0)
            }
            return array
          },
        },
        ...
        dimension: {
          ...
          updateContent: (
            ...
            for (let i = 0; i < array.length; i++) {
              const v = Number(array[i].value) || 0
              sum += v
              array[i].value = formatTooltipValWithUnit(v)
            }
            array.unshift({
              key: tt('Total:'),
              value: formatTooltipValWithUnit(sum),
            })
            return array
          },
        },
